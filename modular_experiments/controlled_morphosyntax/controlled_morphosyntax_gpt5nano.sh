...
## Chihiro: it seems like `storydir` and `story` were the other way round in the original script,
## leading to FileNotFoundError when running the script.
MODEL=gpt5nano
...
python3 modular_experiments/run_morphosyntax.py \
	...
	--storydir=grammatical_test_sentences \
	--story=sentence_design_output \
	--use_safe_params \
	--num_iter=3 \
